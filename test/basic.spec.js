...
describe("Math.round()", () => {
  it("소숫점 이하 값이 0.5 이상이면 올림 값을 반환한다", () => {
    ...
    expect(Math.round(2.6)).toEqual(3);
...
describe("Array.prototype.slice()", () => {
  ...
  it("인자가 하나이면, 주어진 인덱스부터 마지막 인덱스까지의 배열을 반환한다.", () => {
    ...
    expect(nums.slice(3)).toEqual([4, 5]);
  ...
  it("인자가 두 개이면, 인덱스가 첫 번째 인자와 두 번째 인자 사이에 해당하는 배열을 반환한다.", () => {
    ...
    expect(nums.slice(1, 4)).toEqual([2, 3, 4]);
    expect(nums.slice(2, 3)).toEqual([3]);
  });

  it("반환되는 배열은 기존 배열과 다른 새로운 배열이다.", () => {
    expect(nums.slice()).not.toBe(nums);
    expect(nums.slice(0)).not.toBe([1, 2, 3, 4, 5]);
    expect(nums.slice(2)).not.toBe([3, 4, 5]);
    expect(nums.slice(0, 5)).not.toBe([1, 2, 3, 4, 5]);
    expect(nums.slice(0, 4)).not.toBe([1, 2, 3, 4]);
  });
});
